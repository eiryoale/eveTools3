...
namespace std {
	...
	struct hash<pair<int, int>>
	{
		...
		{
			return ((k.first) << 10) | (k.second); // мы знаем, что этот хэш исопльзуетс€ дл€ клеток, и их значени€ вполне предсказуемы.
		}
	};
}
struct circle {
...
auto findPath(const point& start, const point& dest, double step = 20.0) {
	...
	// !!!!!!!!!!! goal может быть недостижима. Ќадо как минимум осматривать несколько соседних точек.
